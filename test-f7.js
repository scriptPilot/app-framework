...
// Pass this if window is not defined yet
  ...
  window.Framework7 = function (params) {
    ...
    app.openModal = function (modal, animated) {
      ...
      if (modal.data('f7-modal-shown') === true) {
        return
      }
    ...
    app.showTab = function (tab, tabLink, animated, force) {
      ...
      if (isSwipeableTabs) {
        ...
        if (window.Swiper.activeIndex !== newTab.index()) window.Swiper.slideTo(newTab.index(), animated ? undefined : 0, false)
      }
    ...
    app.initPageSwiper = function (pageContainer) {
      ...
      swipers.each(function () {
        ...
        var params
        if (window.Swiper.hasClass('tabs-swipeable-wrap')) {
          window.Swiper.addClass('swiper-container').children('.tabs').addClass('swiper-wrapper').children('.tab').addClass('swiper-slide')
          initialSlide = window.Swiper.children('.tabs').children('.tab.active').index()
        }
        if (window.Swiper.data('swiper')) {
          params = JSON.parse(window.Swiper.data('swiper'))
        } else {
          params = window.Swiper.dataset()
        }
    ...
    var PhotoBrowser = function (params) {
      ...
      pb.open = function (index) {
        ...
        if (pb.opened && pb.swiper) {
          pb.window.Swiper.slideTo(index)
          return
        }
        pb.opened = true
        pb.openIndex = index
      ...
      pb.onSliderTransitionStart = function (swiper) {
        pb.activeIndex = window.Swiper.activeIndex

        var current = window.Swiper.activeIndex + 1
        var total = window.Swiper.slides.length
        if (pb.params.loop) {
          total = total - 2
          current = current - window.Swiper.loopedSlides
        ...
        if (window.Swiper.isBeginning && !pb.params.loop) {
          $('.photo-browser-prev').addClass('photo-browser-link-inactive')
        }
        if (window.Swiper.isEnd && !pb.params.loop) {
        ...
        if (pb.captions.length > 0) {
          ...
          var captionIndex = pb.params.loop ? window.Swiper.slides.eq(window.Swiper.activeIndex).attr('data-swiper-slide-index') : pb.activeIndex
          pb.captionsContainer.find('[data-caption-index="' + captionIndex + '"]').addClass('photo-browser-caption-active')
        }

                // Stop Video
        var previousSlideVideo = window.Swiper.slides.eq(window.Swiper.previousIndex).find('video')
      ...
      pb.swipeCloseTouchMove = function (swiper, e) {
        ...
        if (!swipeToCloseStarted) {
          ...
          swipeToCloseActiveSlide = pb.window.Swiper.slides.eq(pb.window.Swiper.activeIndex)
        ...
        pb.window.Swiper.container.css('opacity', opacity).transition(0)
      }
      pb.swipeCloseTouchEnd = function (swiper, e) {
        ...
        pb.window.Swiper.container.css('opacity', '').transition('')
  ...
  Dom7.fn.animate = function (props, params) {
    ...
    =========================== */
  window.Framework7.prototype.support = (function () {
    var support = {
    ...
    =========================== */
    /*
    window.Framework7.prototype.device = (function () {
        var device = {};
