/*
  ...
  - Exit with error (status code 1):
    ...
    - Second parameter is "issue"
  - Exit without error (status code 0):
    - Second parameter is "exit"

  alert(message <string>, [action <"error"/"issue"/"exit">])
...
module.exports = function (msg, action) {
  ...
  // Attach GitHub issue information
  if (action === 'issue') {
    msg += '\n' +
           '\n' +
  ...
  // Abort script
  if ((isError === true || action === 'error' || action === 'exit' || action === 'issue') && action !== 'noexit') {
