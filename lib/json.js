...
let createDocumentation = function (scheme, parentStr) {
  parentStr = parentStr !== undefined ? parentStr + '.' : ''
  let table = parentStr === '' ? 'Option | Allowed | Default' + "\n" + ':--- |:--- |:---' + "\n" : ''
  for (let item in scheme) {
    let option = parentStr + item
    let type = scheme[item].type !== undefined ? scheme[item].type : (scheme[item].props !== undefined ? 'object' : 'text')
    let allowed = scheme[item].allow !== undefined ? scheme[item].allow.join(', ') : ''
    allowed = allowed !== '' ? allowed : '*' + type + '*'
    ...
    if (scheme[item].props !== undefined) {
      table += createDocumentation(scheme[item].props, parentStr + item)
    }
  }
  return table
}
...
module.exports = {
  ...
  docu: function (scheme) {
    ...
    // Validaton ok
      ...
      return createDocumentation(scheme)
    }
  }
}
