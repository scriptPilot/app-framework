...
try {
  // Load file
  let fileName = abs(env.app, 'routes.json')
  let routes = []
  if (found(fileName)) {
    routes = fs.readJsonSync(fileName)
  }
  // Add missing page routes
  rec(abs(env.app, 'pages'), function (err, pageFiles) {
    for (let p = 0; p < pageFiles.length; p++) {
      let pageFile = pageFiles[p].substr(abs(env.app, 'pages').length + 1)
      ...
      if (componentFound === false) {
        routes.push({
          path: '/' + pageFile.replace(/\.vue$/, '') + '/',
          component: pageFile
        })
      }
    }
    // Create/update file
    fs.writeJsonSync(fileName, routes)
  })
} catch (err) {
  alert('Failed to update the routes.json file.' + err, 'issue')
}
