...
let createConfiguration = function (mode) {
  ...
  let loaders = [
    ...
    {
      test: /\.css$/,
      loader: mode === 'development' ? 'css-loader' : ExtractTextPlugin.extract('css-loader' + (env.cfg.buildSourcemaps === true ? '?sourceMap' : ''))
    },
    // Image files
    {
      test: /\.(png|jpe?g|gif)(\?.*)?$/,
  ...
  if (mode === 'production') {
    ...
    let iosSplashIcons = fs.readdirSync(abs(env.cache, 'icons/dev'))
    for (let i = 0; i < iosSplashIcons.length; i++) {
      let iconInfo = iosSplashIcons[i].match(/^ios-splash-([0-9]+)x([0-9]+)\.png$/)
      if (iconInfo !== null) {
        iconTags += '<link rel="apple-touch-startup-image" media="(device-width: ' + iconInfo[1] + 'px) and (device-height: ' + iconInfo[2] + 'px) and (orientation: ' + (iconInfo[1] > iconInfo[2] ? 'landscape' : 'portrait') + ') and (-webkit-device-pixel-ratio: 3)" href="icons/' + iconInfo[0] + '">'
      }
    }
  }
