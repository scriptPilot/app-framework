...
let createConfiguration = function (mode) {
  ...
  if (mode !== 'development' && mode !== 'production') {
    alert('Error: Webpack configuration needs "production" or "development" as parameter.', 'issue')
  }
  ...
  let loaders = [
    ...
    {
      test: /\.css$/,
      loader: mode === 'development' ? 'vue-style-loader!css-loader' : ExtractTextPlugin.extract('css-loader')
    },
    // Image files
    {
      test: /\.(png|jpe?g|gif)(\?.*)?$/,
  ...
  if (mode === 'production') {
    ...
    config.vue = {
      loaders: ExtractTextPlugin.extract('vue-style-loader', 'css-loader')
    }
  }

  // Define icon tags for production
  let iconTags = '<meta name="theme-color" content="' + env.cfg.iconBackgroundColor + '" />' +
