...
mixins.manageApplicationFrame = {
  ...
  methods: {
    updatePhoneFrame: function () {
      ...
      let $$ = window.Dom7
      // Update classes
      if (mode === 'phoneFrame') $$('body').addClass('phoneFrame')
        else $$('body').removeClass('phoneFrame')
      if (mode === 'limitBoth' || mode === 'limitHeight') $$('body').addClass('limitHeight')
        else $$('body').removeClass('limitHeight')
      if (mode === 'limitBoth' || mode === 'limitWidth') $$('body').addClass('limitWidth')
        else $$('body').removeClass('limitWidth')
      // Update size and position
      if (mode === 'limitBoth') {
        $$('#frame').css('height', appMaxHeight + 'px')
        $$('#frame').css('width', appMaxWidth + 'px')
        $$('#frame').css('top', ((windowHeight - appMaxHeight) / 2) + 'px')
        $$('#frame').css('left', ((windowWidth - appMaxWidth) / 2) + 'px')
      } else {
    ...
    f7Ready: function () {
      ...
      function rememberViews () {
        window.Dom7(document).on('page:init page:reinit swipeback:beforechange', e => {
          ...
          // Get urls
          let urls = JSON.parse(JSON.stringify(view.history))
