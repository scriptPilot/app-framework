/* Purpose: Client code entry point */
...
let Vue = process.env.NODE_ENV === 'development' ? require('vue/dist/vue.common.js') : require('vue/dist/vue.common.min.js')
Vue.use(require(process.env.NODE_ENV === 'development' ? '../vendor/framework7-vue/framework7-vue.js' : '../vendor/framework7-vue/framework7-vue.min.js'))
...
function manageApplicationState (app, theme, colors) {
...
function initFirebase (app, callback) {
  // Use Firebase
  if (process.env.USE_FIREBASE_APP === 'true') {
    // Include firebase
    let firebase = require('firebase/app')
    // Init Firebase
    let firebaseConfig = process.env.NODE_ENV === 'production' ? app.config.firebase : app.config.devFirebase
    ...
    if (process.env.USE_FIREBASE_AUTH === 'true') {
      // Include firebase auth service
      require('firebase/auth')
      // Update initial local storage
      app.user = window.user = window.localStorage.user !== undefined ? JSON.parse(window.localStorage.user) : null
      ...
      firebase.auth().onAuthStateChanged(function (newUser) {
        if (newUser) {
          ...
          window.localStorage.user = JSON.stringify(app.user)
        } else {
          app.user = window.user = null
    ...
    if (process.env.USE_FIREBASE_DATABASE === 'true') {
      ...
      window.db = window.timestamp = null
    }
    // Storage service
...
new Vue({ // eslint-disable-line
  ...
  data: {
    ...
    config: config,
    user: null,
    language: window.localStorage.language !== undefined ? window.localStorage.language : config.defaultLanguage,
    // Theme settings
    theme: theme,
    colors: colors,
    color: null,
    ...
    onF7Init: function () {
      ...
      initFirebase(app, function () {
        ...
        if (window.cordova !== undefined) {
          app.appMode = 'native'
        } else if (app.$f7.device.webView !== null || window.matchMedia('(display-mode: standalone)').matches) {
          app.appMode = 'homescreen'
        } else if (app.$f7.device.ios !== false || app.$f7.device.android !== false) {
          app.appMode = 'mobile'
        } else {
          app.appMode = 'desktop'
        }
        ...
        // Manage phone frame
        managePhoneFrame()
        addLoginScreen(function () {
          // Manage application state
          manageApplicationState(app, theme, colors)
          // Check App Framework updates (in development only)
          checkFrameworkUpdates(app)
