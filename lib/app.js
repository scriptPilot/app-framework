      ...
      if (this.config.theme.split('-').indexOf(newTheme) > -1) {
        ...
        if (oldTheme === null) {
          if (process.env.NODE_ENV === 'development') {
            if (newTheme === 'material') require('./material')
            else require('./ios')
...
let manageStatusbar = {
  ...
  watch: {
    f7init: function () {
      ...
      this.statusbarBackgroundColor = window.localStorage.statusbarTextColor
      this.statusbarVisibility = window.localStorage.statusbarTextColor
    },
