...
mixins.loadRoutes = {
  created: function () {
    ...
    for (let r = 0; r < routes.length; r++) {
      ...
      // Login required?
      if (routes[r].login === true) loginRoutes.push(routes[r].path)
      // Tab routes
      if (Array.isArray(routes[r].tabs)) {
        for (let t = 0; t < routes[r].tabs.length; t++) {
          ...
          if (routes[r].tabs[t].login === true) {
            loginRoutes.push(routes[r].path + routes[r].tabs[t].path.substr(1))
          }
          ...
          if (Array.isArray(routes[r].tabs[t].routes)) {
            for (let a = 0; a < routes[r].tabs[t].routes.length; a++) {
              ...
              if (routes[r].tabs[t].routes[a].login === true) {
                loginRoutes.push(routes[r].path + routes[r].tabs[t].path.substr(1) + routes[r].tabs[t].routes[a].path.substr(1))
              }
            }
          }
        }
      ...
      this.$options.framework7.preroute = (view, options) => {
        if (this.user !== null || options.isBack === true || options.url === undefined) {
          ...
          loginRoutes.map(route => {
            let regexp = new RegExp(route.replace(/\/:[a-zA-Z0-9-_]+\//g, '\/[a-zA-Z0-9-]+\/').replace(/^\/(.+)\/$/, '$1')) // eslint-disable-line
            let url = options.url.match(/([0-9a-zA-Z-/]+)/)[1]
            if (url.substr(0, 1) !== '/') url = '/' + url
            if (url.substr(url.length - 1) !== '/') url = url + '/'
            if (regexp.test(url)) loginRequired = true
          })
          if (loginRequired) {
            window.localStorage.requestedPage = options.url
