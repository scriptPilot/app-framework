...
function restoreActions (callback) {
  if (window.localStorage.actions) {
    setTimeout(function () {
      window.Dom7('#app').append('<div class="modal-overlay modal-overlay-visible"></div>')
      window.Dom7('#app').append(window.Dom7(window.localStorage.actions).addClass('modal-in'))
      callback()
    }, 0)
  } else {
    callback()
...
function rememberActions () {
  window.Dom7(document).on('actions:open actions:close', function (e) {
    if (e.type === 'actions:close') {
      window.localStorage.removeItem('actions')
    } else {
      window.localStorage.actions = window.Dom7(e.target).prop('outerHTML')
    }
  })
}
