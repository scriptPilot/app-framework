...
function loadRessources (callback) {
  // Environment
  let db = window.localStorage.db ? JSON.parse(window.localStorage.db) : {}
...
let globalDBObject = {
  ...
  methods: {
    saveData: function (path, value) {
      this.$root.$set(this.$root.db, path, value)
      localStorage.db = JSON.stringify(this.$root.db)
    },
    removeData: function (path) {
      this.$root.$delete(this.$root.db, path)
      localStorage.db = JSON.stringify(this.$root.db)
    }
  }
}
...
function initApp (data, callback) {
  ...
  Vue.mixin(globalDBObject, rememberAndRestoreData)
