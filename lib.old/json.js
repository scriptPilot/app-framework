...
/*
  ...
  // Purpose:
  - Validate object according scheme
...
let createDocumentation = function (scheme, docType, parentStr, listIndent) {
  ...
  for (let item in scheme) {
    ...
    table += listIndent + listIndent + item + ' | ' + allowed + ' | ' + value.replace(/^"(.+)"$/, '$1') + '\n'
...
module.exports = {
  ...
  create: function (scheme, destination) {
    ...
    // Validaton ok
      ...
      if (destination !== undefined) {
        fs.writeJsonSync(destination, object, {spaces: 2})
        return true
      } else {
        return object
      }
  ...
  docu: function (scheme, type) {
    ...
    // Validaton ok
      ...
      return createDocumentation(scheme, type)
    }
  }
}
