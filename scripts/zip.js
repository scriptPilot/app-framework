...
if (cfg.isInstalled) {
  // Show message
  alert('ZIP file creation ongoing - please wait ...')
  ...
  zipdir(cfg.appRoot, {
    ...
    if (!err) {
      alert('Project files and latest build saved to "' + zipFilename + '". You can backup that ZIP file to an external drive or cloud now.')
    } else {
      alert('Error: Zip file creation failed')
    }
  })

// Not installed as package
} else {
  alert('Error: Zip file can be created only after App Framework is installed as a package')
}
